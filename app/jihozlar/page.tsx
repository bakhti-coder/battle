...
import { Search, Trash2, Pencil, Trash } from "lucide-react";
...
export const metadata: Metadata = {
  title: "Jihozlar Page",
  description: "Jihozlar Page || FullStack Battle",
};
...
const TablesPage = () => {
  ...
  const [getJihoz, setGetJihoz] = useState<any>([]);

  const [loading, setLoading] = useState(false);
  const [buttonLoader, setButtonLoader] = useState(false);
  const [turi, setTuri] = useState("Trinajor");
  const [search, setSearch] = useState("");
  const [jihozId, setJihozId] = useState("");

  //////////// Edit data modal value /////////////////
  const [currentJihoz, setCurrentJihoz] = useState<any>("");
  ...
  //////// Get Customers //////////
  useEffect(() => {
    const getJihoz = async () => {
      try {
        setLoading(true);
        const data = await axios.get(URLJIHOZ).then((res) => {
          setGetJihoz(res.data);
        });
      } catch (error) {
        console.log(error);
  ...
  ///////// Send Jihoz ///////////
  const customersSend = async (e: any) => {
    e.preventDefault();
    ...
    try {
      ...
      const response = await axios.post(URLJIHOZ, {
        name: name.value,
        price: price.value,
        brand: brand.value,
        soni: soni.value,
      ...
      toast.success(`Jihoz muvaffaqiyatli qo'shildi`, { autoClose: 1000 });
  ...
  /////// Update Customers ////////////
  const updateJihoz = async (e: any) => {
    e.preventDefault();

    const id = currentJihoz?.id;

    try {
      setButtonLoader(true);
      const response = await axios.put(URLJIHOZ + id, {
        name: currentName,
        price: currentNarxi,
        brand: currentBrand,
        turi: currentTuri,
  ...
  const deleteCustomer = async (id: Number) => {
    try {
      setButtonLoader(true);
      const res = await axios.delete(URLJIHOZ + id).then((res) => {});
  ...
  useEffect(() => {
    if (currentJihoz) {
      setCurrentName(currentJihoz.name);
      setCurrenNarxi(currentJihoz.price);
      setCurrentBrand(currentJihoz.brand);
      setCurrentMalumot(currentJihoz.desc);
      setCurrentSoni(currentJihoz.soni);
      setCurrentTuri(currentJihoz.turi);
    }
  }, [currentJihoz]);
      ...
      {modalUpdate ? (
        ...
        >
          ...
          <form onSubmit={customersSend}>
            <h1 className="text-3xl text-center my-3 text-black font-semibold ">{`Jihoz qo'shish`}</h1>
            <div className="mt-5">
              <input
                type="text"
                name="name"
            ...
            {buttonLoader ? (
              ...
              <button
                type="submit"
                className="py-3 px-12 bg-blue-600 rounded-full text-black font-semibold"
              >{`Jihoz Qo'shish`}</button>
            )}
          </form>
        </ReactModal>
      )}
      <Breadcrumb pageName="Jihozlar" />
      <section>
        ...
        <div className="grid grid-cols-1 bg-white p-7 rounded-lg justify-center sm:justify-start  md:grid-cols-2 lg:grid-cols-3 gap-x-30 gap-y-9">
          {loading ? (
            <Loading />
          ) : (
            getJihoz
              .filter((jihoz: any) => {
                return search.toLowerCase() === ""
                  ? jihoz
                  : jihoz.name.toLowerCase().includes(search);
              })
              .map((jihoz: any, i: number) => (
                <div
                  key={i}
                  className="hover:shadow-xl w-full lg:mx-0 mx-5 bg-[#C1BBEB] relative p-3 rounded-2xl hover:scale-105 transition-transform ease-out duration-200 h-full "
                >
                  <div className="absolute right-5 top-5 flex items-center z-9999">
                    <Pencil
                      onClick={() => {
                        openModal();
                        setUpdateModal(true);
                        setCurrentJihoz(jihoz);
                      }}
                      size={18}
                      className="mr-3 cursor-pointer"
                    />

                    {buttonLoader && jihoz.id === jihozId ? (
                      <div role="status">
                        <svg
                          aria-hidden="true"
                  ...
                  <Link href={`/jihozlar/${jihoz.id}`}>
                    <div>
                      <Image
                        src={"/images/jihoz.png"}
                        width={500}
                        height={160}
                        alt="img"
                      />
                      <div className="flex justify-between items-center mt-2">
                        ...
                        <p className="text-[#C11030] font-semibold text-xl">
                          {jihoz.brand}
                        </p>
                      </div>
                      <div className="flex justify-between items-center mt-2">
                        <h4 className="text-[#5C63A3] font-semibold text-xl">
                          {jihoz.desc}
                        </h4>
